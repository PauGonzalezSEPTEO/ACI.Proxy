...
namespace ACI.Proxy.Core.Dtos
{
    public class RoomTypeDto
    {
        ...
        public RoomTypeDto() : this(null) { }

        public RoomTypeDto(string languageCode) => _languageCode = languageCode;

        public ICollection<BuildingDto> Buildings
        {
            get
            {
                return RoomTypesBuildings.Select(x => new BuildingDto(_languageCode)
                {
        ...
        internal ICollection<RoomTypeTranslationDto> Translations { get; set; } = new HashSet<RoomTypeTranslationDto>();
    }
}
