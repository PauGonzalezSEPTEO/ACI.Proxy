...
export class TemplateListingComponent implements OnInit, OnDestroy {  
  ...
  edit(event: { id: number, isReadOnly: boolean }) {
    ...
    this.aTemplate.subscribe((template: Template) => {
      ...
      this.loadProjectsForSelectedCompanies();   
    });
  }
  ...
  loadBuildingsForProjects() {
    const projectIds = this.templateModel.getRelevantProjectIds();
    this.buildings$ = this.buildingService.getByProjectIds(projectIds, this.translate.currentLang);
  ...
  loadProjectsForSelectedCompanies() {
    const selectedCompanyIds = this.templateModel.companies;
    this.projectService.getByCompanyIds(selectedCompanyIds).subscribe(projects => {
      const selectedProjectIds = this.templateModel.projects;
      this.templateModel.projectsList = projects
      this.templateModel.projects = projects
        .filter(project => selectedProjectIds.includes(project.id))
        .map(project => project.id);
      this.loadBuildingsForProjects();     
    });
  }
